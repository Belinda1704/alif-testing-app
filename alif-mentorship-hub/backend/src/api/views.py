...
class StudentApplicationViewSet(viewsets.ModelViewSet):
    ...
    def update_status(self, request, pk=None):
        application = self.get_object()
        user = request.user
        if not hasattr(user, 'mentor_profile') and not user.is_staff:
            return Response({"error": "Only mentors or admins can perform this action."}, status=status.HTTP_403_FORBIDDEN)

        status_choice = request.data.get('status')
        feedback = request.data.get('feedback', '')
        consultation_date = request.data.get('consultation_date', None)

        if status_choice not in dict(application.STATUS_CHOICES).keys():
        ...
        application.save()
        return Response({"success": f"Application {status_choice} successfully."})
